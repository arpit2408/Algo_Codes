	        ...
	        for (int i=nums.length-1;i>=0;i--)
	        {
	            for(int j=0;j<=i;j++)
	            {
	                ...
	                if(nums[j]!=0)
	                {
	                    if(nums[i]%nums[j]==0)
	                    {
	                        if(!temIntegers.contains(nums[j]))
	                        {
	                                temIntegers.add(nums[j]);
	                        }
	                    }
	                }
	            }
	            for(int l=0;l<temIntegers.size();l++)
                {
	            	for(int j=0;j<=l;j++)
	            	{
	            		if(temIntegers.get(j)!=0)
	            		{
		            		 if(temIntegers.get(l)%temIntegers.get(j)!=0)
		            		 {
		            			 removeints.add(temIntegers.get(j));
		            			 break;
		            		 } 
	            		}
	            	}
                }
	            for(int k=0;k<removeints.size();k++)
	            {
	            	temIntegers.remove(removeints.get(k));
	            }
	            if(divIntegers.size()==nums.length)
	               {
	                   break;
	               }
	            if(divIntegers.size()<temIntegers.size())
	            {
	            	divIntegers.clear();
	                divIntegers.addAll(temIntegers);
	                temIntegers.clear();
	            }
	        }
    for(int i=0;i<divIntegers.size();i++)
    {
    	System.out.println(divIntegers.get(i));
    }
